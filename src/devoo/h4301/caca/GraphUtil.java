...
public class GraphUtil implements Graph {
	...
	{	
       ...
       // CREATE AN ORDERED TAB OF DURATIONS
        ...
        private int calculCost(LinkedList<Troncon> itineraire)
        {
            ...
            int i;
            for(i=0;i<Isize;i++){
                cout = cout + (itineraire.get(i).getLongueur()/itineraire.get(i).getVitesse());
                System.out.println((itineraire.get(i).getLongueur()/itineraire.get(i).getVitesse()));
            }
            return (int) cout;
        }
	...
	private int getDuration(int noeud1, int noeud2, Plan P)
	{
		 ...
		 if((tabTroncons.get(i).getOrigine().getId() == noeud1)
                 ...
                 {
                     return (int) (tabTroncons.get(i).getLongueur()/tabTroncons.get(i).getVitesse());
                 }
                i++;
                }
                return -1;
